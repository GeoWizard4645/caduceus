...
pub use computer::{ComputerAction, ComputerController};
pub use types::*;
...
pub fn backend_for(kind: BackendKind) -> Arc<dyn AgentBackend> {
    match kind {
        ...
        BackendKind::Anthropic => Arc::new(anthropic::AnthropicBackend),
    }
}
...
Computer use is not set up yet.\n\n\
It needs a backend that can see your screen and control your mouse \u{2014} today \
that means Claude. In Settings \u{2192} Agent Backends, add a Claude backend with \
your Anthropic API key, tick \u{201c}Allow computer use\u{201d}, then choose it as \
the computer-use backend.";
...
pub fn start_session<R: Runtime>(
    ...
    let ctx = AgentLoopContext {
        ...
        computer: ComputerController::start(
            snapshot.agents.monitor_index,
            snapshot.agents.screenshot_max_dimension,
        ),
        cancel,
        approval,
        max_steps: snapshot.agents.max_steps,
        settle: std::time::Duration::from_millis(snapshot.agents.action_settle_ms),
    };
...
mod tests {
    ...
    fn every_backend_kind_resolves_to_an_implementation() {
        for kind in [BackendKind::Null, BackendKind::OpenAiCompatible, BackendKind::Anthropic] {
            let b = backend_for(kind);
            assert!(!b.id().is_empty());
            assert!(!b.display_name().is_empty());
        }
        assert!(backend_for(BackendKind::Anthropic).supports_computer_use());
    ...
    #[test]
    fn a_fresh_install_resolves_the_primary_backend_to_null() {
        let s = Settings::default();
        let cfg = resolve_backend(&s, BackendRole::Primary).unwrap();
        assert_eq!(cfg.kind, BackendKind::Null);
    }

    #[test]
    fn computer_use_is_unconfigured_by_default_with_a_helpful_message() {
        let s = Settings::default();
