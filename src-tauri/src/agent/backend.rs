...
/// Everything an agent session needs beyond the task text.
pub struct AgentLoopContext {
    pub session_id: String,
    pub on_step: StepSink,
    /// Polled between steps; when it returns true the session unwinds and
    /// reports [`StopReason::UserStopped`](super::types::StopReason::UserStopped).
    pub cancel: CancelToken,
    /// Awaited before anything touches the machine. Resolves to `false` if the
    /// user declines.
