...
pub struct RuntimeInfo {
    ...
    pub suggested_anthropic_models: Vec<String>,
    pub computer_use_note: String,
}

#[tauri::command]
pub fn get_runtime_info<R: Runtime>(
    app: AppHandle<R>,
    settings: tauri::State<'_, SettingsManager>,
) -> RuntimeInfo {
    ...
    RuntimeInfo {
        ...
        backends_with_keys: cfg
            ...
            .map(|b| b.id.clone())
            .collect(),
        suggested_anthropic_models: agent::anthropic::SUGGESTED_MODELS
            .iter()
            .map(|s| s.to_string())
            .collect(),
...
pub async fn agent_list_models(
    ...
    match backend_config.kind {
        settings::BackendKind::OpenAiCompatible => agent::openai::list_models(&backend_config)
            .await
            .map_err(|e| e.user_message()),
        settings::BackendKind::Anthropic => Ok(agent::anthropic::SUGGESTED_MODELS
            .iter()
            .map(|s| s.to_string())
            .collect()),
        settings::BackendKind::Null => Ok(Vec::new()),
    }
}
...
pub fn agent_backend_templates() -> Vec<BackendConfig> {
    vec![
        ...
        settings::anthropic_template(uuid::Uuid::new_v4().to_string()),
    ]
}
