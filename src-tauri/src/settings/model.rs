...
pub struct AgentSettings {
    ...
    /// Hard stop on how many model turns one agent task may take.
    pub max_steps: u32,
    /// Show a confirmation dialog before the *first* action of any
    /// computer-use session. Strongly recommended; defaults to on.
    pub confirm_before_first_action: bool,
    /// Longest side, in pixels, that a screenshot is downscaled to before being
    /// sent to the model. Smaller is cheaper and faster; too small hurts
    /// accuracy. 1280 is a good balance on a Retina display.
    pub screenshot_max_dimension: u32,
    /// Pause inserted after each simulated input action so the UI under
    /// automation has time to settle before the next screenshot.
    pub action_settle_ms: u64,
    /// Which monitor computer-use drives. `0` = primary.
...
impl Default for AgentSettings {
    fn default() -> Self {
        Self {
            ...
            primary_backend_id: Some("null".into()),
            computer_use_backend_id: None,
            max_steps: 25,
...
pub enum BackendKind {
    ...
    /// OpenAI, Ollama, LM Studio, vLLM, llama.cpp, OpenRouter, Groq, …
    OpenAiCompatible,
    /// The Claude Messages API, including the computer-use tool.
    Anthropic,
}
...
pub struct BackendConfig {
    ...
    /// Whether this backend may be selected for computer use.
    pub supports_computer_use: bool,
    /// `anthropic-beta` header value. Fetched from the current docs at build
    /// time; update it here when Anthropic ships a newer one.
    pub anthropic_beta_header: String,
    /// Computer-use tool `type` string, e.g. `computer_20251124`.
    pub computer_tool_version: String,
    /// Allow the model to request a zoomed crop of the screen. Only supported
    /// by `computer_20251124` and later.
    pub enable_zoom: bool,
    /// Extra HTTP headers sent with every request, as `[name, value]` pairs.
    /// Useful for gateways and proxies that require custom auth.
...
impl Default for BackendConfig {
    fn default() -> Self {
        Self {
            ...
            anthropic_beta_header: DEFAULT_ANTHROPIC_BETA.into(),
            computer_tool_version: DEFAULT_COMPUTER_TOOL_VERSION.into(),
            enable_zoom: true,
...
/// Current as of the computer-use docs fetched while building this release.
/// See `docs/ARCHITECTURE.md` → "Keeping up with provider changes".
pub const DEFAULT_ANTHROPIC_BETA: &str = "computer-use-2025-11-24";
pub const DEFAULT_COMPUTER_TOOL_VERSION: &str = "computer_20251124";
pub const DEFAULT_ANTHROPIC_MODEL: &str = "claude-opus-5";
pub const DEFAULT_ANTHROPIC_BASE_URL: &str = "https://api.anthropic.com";

/// A fresh install has exactly one backend: the no-op. It exists so that every
/// AI code path has something to resolve to, and so the UI has a place to
/// explain what to do next instead of erroring.
pub fn default_backends() -> Vec<BackendConfig> {
    vec![BackendConfig {
        id: "null".into(),
        display_name: "Not configured".into(),
        kind: BackendKind::Null,
        ..Default::default()
    }]
}

/// A ready-made Anthropic config, offered by the Settings UI as a one-click
/// starting point. Not installed by default (that would imply an API key).
pub fn anthropic_template(id: impl Into<String>) -> BackendConfig {
    BackendConfig {
        id: id.into(),
