# Build Prompt: "Caduceus" — Personal AI Command Center

> Placeholder name: **Caduceus**. Rename freely (find/replace `caduceus`/`Caduceus` throughout) before or after the build — this has no effect on functionality.
...
- **Hover behavior:** hovering over the staff immediately expands 6 smaller icons in a radial/arc pop-out around it, animated.
- **Auto-collapse:** if the pointer is anywhere else on screen for a configurable idle duration (default 3s), the pop-out (and staff expansion) collapses. Idle duration configurable in Settings (e.g. 1–10s).
- Clicking the **center** staff opens the Command Center (3.2).
...
Design one generic `Shortcut` data model used both for the staff's 6 pop-out icons and for anything discoverable in the Command Center:
...
interface Shortcut {
  ...
  showInStaff?: boolean;      // whether this appears as one of the staff's pop-out icons
  orderIndex?: number;
}
```

- Ship exactly 6 default shortcuts with `showInStaff: true`, matching this brief's original request, but every field user-editable and deletable in Settings, with an "Add Shortcut" flow:
...
- Settings UI must let a user reorder, add, remove, and edit all shortcuts, and choose which subset (up to 6, but don't hard-cap at exactly 6 in the data model — cap the staff's pop-out display at 6 visible, allow more defined) appear on the staff.
...
1. **General** — hotkey for staff visibility toggle, staff position/edge, hover expand delay, auto-collapse idle duration, launch-at-login toggle.
2. **Shortcuts** — full CRUD over the `Shortcut` list from 3.3, reorderable, with "show in staff" checkboxes (max 6 enforced in UI).
...
7. **Appearance** — theme (see Section 8), staff size, accent handling.
...
│   ├── src/
│   │   ├── main.rs
│   │   ├── window/          # staff window, command center window, settings window management
...
- The staff itself should feel alive but not busy: a calm idle state, a satisfying hover/expand animation, clear focus states in the Command Center.
...
1. Scaffold Tauri + React project, get an empty always-on-top transparent staff window rendering and draggable, plus tray icon with a visibility toggle.
2. Implement the `Shortcut` model, Settings persistence layer (`tauri-plugin-store` + keyring wiring), and the staff's hover pop-out with the 6 default shortcuts wired to real `open_url`/`open_app` execution.
...
9. Polish pass on visuals per Section 8; test the full hover → staff → shortcut and staff-center → Command Center → each routing path end to end.
...
- [ ] Staff toggles via tray, hotkey, and Settings; hover pop-out and idle auto-collapse both work and are configurable.
- [ ] Command Center opens from the staff's center, all three default prefix behaviors work, and custom prefixes can be added in Settings.
